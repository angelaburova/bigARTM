...
while tau > left:
    ...
    print theta_score
    plt.plot(model_artm.score_tracker["PerplexityScore"].value)
    if phi_score==1.0 or theta_score==0:
        break;
    elif phi_score >= phi_best_score and theta_score > theta_best_score:
        phi_best_score = phi_score
        theta_best_score = theta_score
        best_topic_names = model_artm.topic_names
        best_last_tokens = model_artm.score_tracker["top_words"].last_tokens
    k = k + 1
    tau = -k * 1e4
print "best phi: "
print phi_best_score
...
for topic_name in best_topic_names:
    print topic_name + ': ',
    for word in best_last_tokens[topic_name]:
        print word,
    print
####################################################
